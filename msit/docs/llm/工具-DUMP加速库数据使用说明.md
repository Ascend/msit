...
| -er，--execute-range           | 指定 dump 的 token 轮次范围，区间左右全闭，可以支持多个区间序列，默认为第 0 次，使用方式：-er 1,3 或 -er 3,5,7,7（代表区间[3,5],[7,7],也就是第 3，4，5，7 次。此外，请确保输入多区间时的总输入长度不超过500个字符。 token）                                                                                                                                                                                                                  | 否   |
