...
* 本工具**不校验**输入文件的权限位、属主一致性及软链接安全性；能否读取由操作系统权限机制决定。
* 用户传入的路径若为软链接，工具会通过路径解析读取目标文件；遍历权重目录时跳过子级软链接，以避免循环引用。
* 工具生成的输出文件与目录（如 `msprechecker_dumped.json`、`msprechecker_env.sh`、`run` 落盘结果等）遵循**当前进程的 umask**。
* 请用户及管理员自行确保输入路径可信、权限配置合理，并妥善管理输出文件的访问范围。
...
| `--output-path`         | 可选 | 预检结果的保存路径，保存文件的格式为`msprechecker_{timestamp}_output.json`。默认值：`''`（表示直接打印预检结果）                                              |
