...
class TestLscpuCollector(unittest.TestCase):
    ...
    def test_parse_output_with_primary_key_should_collect_successfully(
    ...
    @patch("subprocess.check_output")
    def test_parse_output_with_secondary_key_should_collect_successfully(
        self, mock_check_output
    ):
    ...
    @patch("subprocess.check_output")
    def test_parse_output_with_multiple_keys_should_collect_primary(
        self, mock_check_output
    ):
    ...
    @patch("subprocess.check_output")
    def test_parse_output_with_chinese_key_should_collect_successfully(
        self, mock_check_output
    ):
        test_output = "你别管\n型号名称: I Love U"
        mock_check_output.return_value = test_output
...
class TestVirtualMachineCollector(unittest.TestCase):
    ...
    @patch(
        "msprechecker.collectors.sys.open_s",
        new_callable=mock_open,
        read_data="hypervisor",
    )
...
class TestCPUHighPerformanceCollector(unittest.TestCase):
    ...
    def test_psutil_strategy_when_current_freq_eq_max_should_return_true(
        self, mock_cpu_freq
    ):
    ...
    @patch("psutil.cpu_freq")
    def test_psutil_strategy_when_current_freq_not_eq_max_should_return_false(
        self, mock_cpu_freq
    ):
        collector = CPUHighPerformanceCollector(strategies=[PsutilStrategy()])
    ...
    @patch("psutil.cpu_freq")
    def test_psutil_strategy_when_current_freq_not_exists_should_return_false(
        self, mock_cpu_freq
    ):
        collector = CPUHighPerformanceCollector(strategies=[PsutilStrategy()])
    ...
    @patch("subprocess.check_output")
    def test_dmidecode_strategy_when_current_freq_eq_max_should_return_true(
        self, mock_check_output
    ):
...
"""
    ...
    @patch("subprocess.check_output")
    def test_dmidecode_strategy_when_current_freq_not_eq_max_should_return_false(
        self, mock_check_output
    ):
        mock_check_output.return_value = """
Processor Information
...
"""
    ...
    @patch("subprocess.check_output")
    def test_dmidecode_strategy_when_run_failed_should_return_false(
        self, mock_check_output
    ):
    ...
    @patch("subprocess.check_output")
    def test_cpupower_strategy_when_current_freq_eq_max_should_return_true(
        self, mock_check_output
    ):
        mock_check_output.return_value = """
...
"""
    ...
    @patch("subprocess.check_output")
    def test_cpupower_strategy_when_current_freq_not_eq_max_should_return_false(
        self, mock_check_output
    ):
        mock_check_output.return_value = """
hardware limits: 1.20 GHz - 3.00 GHz
...
"""
    ...
    @patch("subprocess.check_output")
    def test_cpupower_strategy_when_run_failed_should_return_false(
        self, mock_check_output
    ):
    ...
    @patch("subprocess.check_output")
    def test_cpupower_strategy_when_run_unexpected_should_return_false(
        self, mock_check_output
    ):
        mock_check_output.return_value = """
hardware limits: 1.20 GHz - 3.00 GHz
"""
    ...
    @patch("subprocess.check_output")
    def test_lshw_strategy_when_current_freq_eq_max_should_return_true(
        self, mock_check_output
    ):
        mock_check_output.return_value = """
size: 1
...
"""
    ...
    @patch("subprocess.check_output")
    def test_lshw_strategy_when_current_freq_not_eq_max_should_return_false(
        self, mock_check_output
    ):
...
"""
    ...
    @patch(
        "msprechecker.collectors.sys.open_s",
        new_callable=mock_open,
        read_data="performance",
    )
    def test_scaling_governor_strategy_should_return_true_when_all_cpu_is_performance(
        self, mock_open_s
    ):
    ...
    @patch(
        "msprechecker.collectors.sys.open_s", new_callable=mock_open, read_data="不知道"
    )
    def test_scaling_governor_strategy_should_return_false_when_any_cpu_is_not_performance(
        self, mock_open_s
    ):
        collector = CPUHighPerformanceCollector(strategies=[ScalingGovernorStrategy()])
        result = collector.collect()
    ...
    def test_scaling_governor_strategy_should_return_false_when_no_cpu_found(
        self, mock_cpu_count
    ):
        collector = CPUHighPerformanceCollector(strategies=[ScalingGovernorStrategy()])
        result = collector.collect()
    ...
    def test_scaling_governor_strategy_should_return_false_when_open_failed(
        self, mock_open_s
    ):
...
class TestKernelInfoCollector(unittest.TestCase):
    ...
    @patch(
        "msprechecker.collectors.sys.open_s",
        new_callable=mock_open,
        read_data="[always] madvise never",
    )
...
class TestMemoryInfoCollector(unittest.TestCase):
    ...
    @patch("os.sysconf")
    @patch("msprechecker.collectors.sys.open_s")
    def test_when_overcommit_file_fails_when_overcommit_memory_path_open_failed(
        self, mock_file, mock_sysconf
    ):
        mock_sysconf.return_value = 4096
...
class TestSysCollector(unittest.TestCase):
    def test_collect_all_info(self):
        ...
        vm_mock.collect.return_value = MagicMock(
            data={"virtual_machine": "False"}, error_handler=CollectErrorHandler()
        )
        ...
        kernel_mock.collect.return_value = MagicMock(
            data={"system": "Linux"}, error_handler=CollectErrorHandler()
        )

        mem_mock = MagicMock()
        mem_mock.collect.return_value = MagicMock(
            data={"page_size": 4096}, error_handler=CollectErrorHandler()
        )
