...
class TestIsInContainer(unittest.TestCase):
    ...
    @patch("os.path.exists")
    @patch(
        "msprechecker.utils.helper.open_s",
        new_callable=mock_open,
        read_data="systemd (1, #threads: 1)\n",
    )
    def test_is_in_container_when_dockerenv_not_exists_and_systemd(
    ...
    @patch(
        "msprechecker.utils.helper.open_s",
        new_callable=mock_open,
        read_data="not_systemd (1, #threads: 1)\n",
    )
    def test_is_in_container_when_dockerenv_not_exists_and_not_systemd(
        self, mock_open_file, mock_exists
    ):
    ...
    def test_is_in_container_when_sched_file_read_fails(
        self, mock_open_file, mock_exists
    ):
