...
from ._test import RuleTestRunner
from ._test import make_test_suite
from .data_source import DataSource
from .data_source import NAType
from .parser import Parser
from .util import LOG_LEVELS
from .util import cmate_logger
from .util import load
from .visitor import AssignmentProcessor
from .visitor import ASTFormatter
from .visitor import EnvironmentScriptGenerator
from .visitor import InfoCollector
from .visitor import RuleCollector
...
def _format_field(label, value, indent=0):
    cmate_logger.info("%s%s : %s", "  " * indent, label, value)
    ...
    for target, (input_path, input_parse_type) in input_targets.items():
        ...
        if target == "env":
            ...
            try:
                ...
                cmate_logger.error(
                    "Failed to parse configuration '%s' from: '%s'. The %s syntax appears to be invalid. Error: %s",
                    target,
                    input_path,
                    parse_type,
                    e,
...
def _log_missing_error(missing_deps, all_targets, all_contexts):
        ...
        if missing_contexts:
            ...
            for missing_context in missing_contexts:
                if missing_context in all_contexts:
                    ...
                    desc = (
                        context_info["desc"][0]
                        if isinstance(context_info["desc"], tuple)
    ...
    error_parts.append(
        ...
        "  - Use '-c <target-name>:<file-path>' to specify missing configuration targets\n"
        "  - Use '-C <context-name>:_format_field<value>' to provide required context variables"
    )
...
def parse(rule_path):
    ...
    with Path(rule_path).open(encoding="utf-8") as f:
...
def inspect(rule_path: str, output_format: str):
    output_format_map = {"text": _display_text, "json": _display_json}
...
def main():
    ...
    run_parser.add_argument("rule", type=readable_file, help="Path to the rule definition file (CMATE format)")
    ...
    run_parser.add_argument("--output-path", help="Path to the save msprechecker output.")
    ...
    run_parser.add_argument("-l", "--log-level", choices=LOG_LEVELS, default="info", help="Set the logging level.")
    ...
    inspect_parser.add_argument("rule", type=readable_file, help="Path to the rule definition file (CMATE format)")
