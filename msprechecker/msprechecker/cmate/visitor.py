...
class _ExpressionEvaluator(NodeVisitor):
    def __init__(self, data_source):
        ...
        self.func_map = {**self.builtin_fn, **self._load_custom_fn()}
        self.history = None
    ...
    def _load_custom_fn():
        ...
        for name in dir(custom_fn):
            ...
            if inspect.isfunction(obj) and not name.startswith("_") and obj.__module__ == custom_fn.__name__:
...
class InfoCollector(NodeVisitor):
    ...
    def visit_meta(self, node: _ast.Meta):
        if self._metadata_map:
            cmate_logger.warning("Multiple metadata declarations found. Previous declaration will be overwritten.")
            self._metadata_map = {}
    ...
    def visit_dependency(self, node: _ast.Dependency):
        if self._dependency_map:
            cmate_logger.warning("Multiple dependency declarations found. Previous declaration will be overwritten.")
            self._dependency_map = {}
        ...
        for desc_node in node.body:
            ...
            if name in self._dependency_map:
                cmate_logger.warning(
                    "Name '%s' is redefined at line %d, column %d. Previous definition will be overwritten.",
                    name,
                    desc_node.lineno,
                    desc_node.col,
                )
    ...
    def visit_partition(self, node: _ast.Partition):
        ...
        if self._namespace in self._partition_map:
            cmate_logger.warning(
                "Multiple partition target %s declarations found. Previous declaration will be overwritten.",
                self._namespace,
            )
    ...
    def collect(self, node):
        try:
            ...
            raise CMateError(f"CMATE configuration parsing failed at position: {e!s}") from e

        for target, partition in self._partition_map.items():
            for required_item in ("required_targets", "required_contexts"):
                if partition[required_item] is not None:
                    partition[required_item] = list(partition[required_item])

        contexts = {
            ctx_var: {"desc": self._dependency_map.get(ctx_var), "options": list(options)}
            for ctx_var, options in self._context_map.items()
        }

        return {"metadata": self._metadata_map, "targets": self._partition_map, "contexts": contexts}
...
class EnvironmentScriptGenerator(NodeVisitor):
    ...
    def visit_compare(self, node: _ast.Compare):
        ...
        if node.op == "==":
            ...
            comparator_val = self._evaluator.evaluate(node.comparator)
            if not isinstance(comparator_val, (list, tuple)):
                return
            expected_val = next(iter(comparator_val))
        else:
            return
    ...
    def generate(self, node):
        ...
        with Path("set_env.sh").open("w", encoding="utf-8") as f:
            self._set_scripts = self._set_scripts or [":"]
            self._undo_scripts = self._undo_scripts or [":"]

            script = script_format.format(set="\n    ".join(self._set_scripts), undo="\n    ".join(self._undo_scripts))
            f.write(script)
