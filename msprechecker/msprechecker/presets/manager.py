...
from ..utils import ErrorSeverity, get_npu_count, get_npu_type, logger, NpuType
...
class RuleManager:
    ...
    def create_rule(type_, value, reason="", severity=ErrorSeverity.ERR_HIGH):
        return {
            "expected": {"type": type_, "value": value},
            "reason": reason,
            "severity": severity,
        }
    ...
    def _get_custom_rules(self):
        ...
        try:
            ...
            logger.warning(
                "'--custom-config-path' passed an insecure path, skipped\n%s", e
            )
            return {}
