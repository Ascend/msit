...
class BaseChecker(ABC):
    def __init__(
        self, *, error_handler: ErrorHandler = None, rule_manager: RuleManager = None
    ):
    ...
    def check(self, collect_result) -> BaseError:
...
class NodeChecker(BaseChecker):
    ...
    def _get_temp_variables(ref_section, visited_nodes):
        ...
        for ref_item in ref_section:
            ...
            if "as" not in ref_item:
                raise WaMLError(
                    "Reference definition must contain 'as' field for variable naming"
                )
            ...
            for i, _ in enumerate(visited_nodes[scope]):
                ...
                processed_condition = MacroExpander.expand(
                    condition, local_path, visited_nodes
                )
                ...
                if attribute is None:
                    ...
                    processed_attribute = MacroExpander.expand(
                        attribute, local_path, visited_nodes
                    )

                temporary_variables[full_var_name] = Evaluator.evaluate(
                    processed_attribute
                )
    ...
    def _process_search_block(self, block, path, visited_nodes):
        ...
        if not isinstance(actual, list):
            raise WaMLError(
                f"Expected a list for search operation, got {type(actual).__name__}"
            )
        ...
        if "len" in block:
            ...
            if isinstance(expected_length, int):
                ...
                expected_expr = MacroExpander.expand(
                    expected_length, path, visited_nodes
                )
    ...
    def _process_conditional_block(self, block, path, visited_nodes):
        ...
        result = self._process_block(
            block=next_block, path=path, visited_nodes=visited_nodes
        )
        return result
    ...
    def _process_basic_block(self, block, path, visited_nodes):
        ...
        if isinstance(case, dict):
            if "type" not in case or "value" not in case:
                raise WaMLError(
                    "Basic block must contain 'type' and 'value' fields if 'case' if a dict type"
                )
            ...
            return (
                validator.validate(actual, evaluated_value),
                reason,
                severity,
                evaluated_value,
            )
