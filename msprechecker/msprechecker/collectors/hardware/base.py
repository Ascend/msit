...
class BaseStressCollector(BaseCollector):
    ...
    def _matmul_stress_test(self, device_type, device_id):
        ...
        batch_size, seq_len, hidden_size, intermediate_size = self._get_shape_by_type(
            check_type
        )
        ...
        for _ in range(10):
            ...
            mat_b = self.torch.randn(batch_size, hidden_size, intermediate_size).to(
                device_pos
            )
            mat_c = self.torch.randn(seq_len, intermediate_size).to(device_pos)
            self.torch.addbmm(mat_c, mat_a, mat_b)

    def _check_memory_for_matmul(
        self, batch_size, seq_len, hidden_size, intermediate_size, device_pos
    ):
