...
from msmodelslim.quant.quantizer.activation.observer import PerTensorObserver, PerTensorConfig


class TestPerTensorObserver(unittest.TestCase):
    def setUp(self):
        self.config = PerTensorConfig()
        self.strategy = MinMaxStrategy()
        self.observer = PerTensorObserver(config=self.config)
