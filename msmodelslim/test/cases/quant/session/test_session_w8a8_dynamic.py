...
from msmodelslim.quant.processor.quant.w8a8_dynamic import W8A8DynamicProcessorConfig, W8A8DynamicLinearFakeQuantizer
from msmodelslim.quant.processor.quant.w8a8_dynamic import W8A8DynamicQuantConfig
...
class TestW8A8DynamicQuantization(unittest.TestCase):
    """测试W8A8动态量化功能的单元测试类"""
    ...
    def setUp(self):
        ...
        self.model = nn.Sequential(
            ...
            nn.LayerNorm([20]),
            nn.Linear(20, 5, dtype=torch.float32, bias=True)
        )

        # 创建W8A8动态量化配置
        self.w8a8_dynamic_config = W8A8DynamicQuantConfig()

        W8A8DynamicQuantConfig.model_validate(self.w8a8_dynamic_config)
        ...
        self.session_config = SessionConfig(
            processor_cfg_map={
                "w8a8_dynamic": W8A8DynamicProcessorConfig(
                    cfg_map={
                        "*": self.w8a8_dynamic_config
                    }
                ),
                "save": SaverProcessorConfig(
                    save_output_path=self.temp_dir,
                    safetensors_name="w8a8_dynamic_model.safetensors",
                    json_name="w8a8_dynamic_config.json",
    ...
    def test_w8a8_dynamic_quantization_basic(self):
        """测试基本的W8A8动态量化功能"""
        # 记录原始模型的权重
        original_weights = []
        ...
        for name, _ in original_weights:
            self.assertIsInstance(self.model.get_submodule(name), W8A8DynamicLinearFakeQuantizer)
        ...
        # 验证模型文件是否被保存
        import os
        self.assertTrue(os.path.exists(os.path.join(self.temp_dir, "w8a8_dynamic_model-00001-of-00001.safetensors")))
        self.assertTrue(os.path.exists(os.path.join(self.temp_dir, "w8a8_dynamic_config.json")))

        # 检查json内容
        with open(os.path.join(self.temp_dir, "w8a8_dynamic_config.json"), "r") as f:
        ...
        with safe_open(os.path.join(self.temp_dir, "w8a8_dynamic_model-00001-of-00001.safetensors"),
                       framework="pt") as f:
            for key in f.keys():
