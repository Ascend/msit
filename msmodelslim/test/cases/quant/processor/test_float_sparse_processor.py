...
from msmodelslim.quant.ir import W16A16sLinear
from msmodelslim.quant.processor.sparse.float_sparse import FloatSparseProcessor, FloatSparseProcessorConfig
from msmodelslim.quant.processor.sparse.admm import AdmmPruner, quantize_l2, quantize_clip, split_half
...
class TestFloatSparseProcessor(TestProcessorBase):
    ...
    def setUp(self):
        ...
        self.assertGreater(len(self.linear_layer_names), 0,
                           "Model should have at least one linear layer")
    ...
    def test_sparse_all_linear_layers(self):
        ...
        for layer_name in self.linear_layer_names:
            ...
            self.assertIsInstance(layer, W16A16sLinear,
                                  f"Layer {layer_name} should be W16A16sLinear")
    ...
    def test_sparse_specific_layers(self):
        ...
        config = self.create_processor_config(
            sparse_ratio=0.4, include=[target_layer])
        ...
        self.assertIsInstance(target_module, W16A16sLinear,
                              f"Target layer {target_layer} should be W16A16sLinear")

        # 验证其他层未被稀疏化
        for layer_name in self.linear_layer_names[1:]:
            layer = self.get_module_by_name(self.model, layer_name)
            self.assertIsInstance(
                layer, nn.Linear, f"Layer {layer_name} should remain as Linear")
    ...
    def test_exclude_specific_layers(self):
        ...
        config = self.create_processor_config(
            sparse_ratio=0.3, include=["*"], exclude=[excluded_layer])
        ...
        self.assertIsInstance(excluded_module, nn.Linear,
                              f"Excluded layer {excluded_layer} should remain as Linear")
    ...
    def test_different_sparse_ratios(self):
        ...
        for sparse_ratio in sparse_ratios:
            with self.subTest(sparse_ratio=sparse_ratio):
                ...
                config = self.create_processor_config(
                    sparse_ratio=sparse_ratio, include=["*"])
                self.run_processor_with_cfg(config)
    ...
    def test_warning_unmatched_include_patterns(self):
        ...
        with self.assertLogs('msmodelslim.quant.processor.sparse.float_sparse', level='WARNING') as log_context:
            self.run_processor_with_cfg(config)

        log_messages = log_context.output
        self.assertTrue(
            any("include patterns are not matched" in msg for msg in log_messages))
        self.assertTrue(
            any("nonexistent_layer" in msg for msg in log_messages))
        self.assertTrue(
            any("another_fake_layer" in msg for msg in log_messages))
    ...
    def test_warning_unmatched_exclude_patterns(self):
        ...
        log_messages = log_context.output
        self.assertTrue(
            any("exclude patterns are not matched" in msg for msg in log_messages))
        self.assertTrue(
            any("nonexistent_layer" in msg for msg in log_messages))
        self.assertTrue(
            any("another_fake_layer" in msg for msg in log_messages))
    ...
    def test_warning_mixed_matched_unmatched_patterns(self):
        ...
        self.assertIsInstance(existing_module, W16A16sLinear,
                              f"Layer {existing_layer} should be W16A16sLinear")
    ...
    def test_no_warning_with_valid_patterns(self):
        ...
        existing_layer = self.linear_layer_names[0]
        config = self.create_processor_config(
            sparse_ratio=0.3, include=[existing_layer])
    ...
    @patch('msmodelslim.quant.processor.sparse.float_sparse.AdmmPruner')
    def test_admm_pruner_creation_and_cleanup(self, mock_admm_pruner_class):
        ...
        # 排除lm_head层
        config = self.create_processor_config(
            sparse_ratio=0.3, include=["*"], exclude=["lm_head"])
        self.run_processor_with_cfg(config)
...
class TestQuantizeL2(unittest.TestCase):
    ...
    def test_quantize_l2_basic_functionality(self):
        ...
        self.assertTrue(torch.allclose(
            original_mant[0, 0].abs(), result_mant[0, 0].abs(), atol=1e-3))
    ...
    def test_quantize_l2_different_thresholds(self):
        ...
        for threshold in thresholds:
            result = quantize_l2(
                keep_mask, para, threshold=threshold, rtn=True)
    ...
    def test_quantize_l2_keep_mask_effect(self):
        ...
        result_empty = quantize_l2(
            keep_mask_empty, para, threshold=2, rtn=True)
    ...
    def test_quantize_l2_extreme_values(self):
        ...
        if torch.any(finite_mask):
            self.assertTrue(torch.all(torch.isfinite(
                result[finite_mask]) | (result[finite_mask] == 0)))
    ...
    def test_quantize_l2_negative_values(self):
        ...
        # 验证符号保持
        self.assertTrue(torch.all(torch.sign(result) ==
                        torch.sign(para.to(torch.half))))
...
class TestSplitHalf(unittest.TestCase):
    ...
    def test_split_half_infinite_values(self):
        """测试无穷大值处理"""
        para = torch.tensor(
            [float('inf'), float('-inf'), 1.0], dtype=torch.float16)
