...
from msmodelslim.quant.ir import W8A16PerChannelFakeQuantLinear
...
def check_w8a16_static_export(module: W8A16PerChannelFakeQuantLinear, name: str,
                             all_tensors: Dict[str, torch.Tensor]) -> None:
    """检查W8A16PerChannelFakeQuantLinear模块的导出内容"""
    # 检查权重相关tensor
...
def check_w8a16_dynamic_export(module: W8A16PerChannelFakeQuantLinear, name: str,
                              all_tensors: Dict[str, torch.Tensor]) -> None:
...
@pytest.mark.smoke
def test_w8a16_per_channel_quantization(test_device, test_dtype):
    """测试W8A16 per_channel量化功能（act: per_tensor, weight: per_channel）"""
    torch.set_default_dtype(test_dtype)
    tmp_dir = tempfile.mkdtemp()

    try:
        # 执行per_channel量化测试（w8a16-per-channel.yaml使用per_tensor+per_channel）
        model_adapter = invoke_test("w8a16_per_channel.yaml", tmp_dir)
        ...
        check_quant_model_description(tmp_dir, "W8A16")
        ...
        # 验证每个W8A16PerChannelFakeQuantLinear模块的导出内容
        for name, module in quantized_model.named_modules():
            if isinstance(module, W8A16PerChannelFakeQuantLinear):
                check_w8a16_static_export(module, name, all_tensors)
