...
@QABCRegistry.multi_register(
    dispatch_key=[
        (fp16_placeholder_sym, int4_per_channel_sym),
        (fp16_placeholder_sym, int4_per_channel_asym),
        (fp16_placeholder_asym, int4_per_channel_sym),
        (fp16_placeholder_asym, int4_per_channel_asym),
    ],
    abc_type=AutoFakeQuantLinear
)
@logger_setter('msmodelslim.quant.ir.w4a16')
class W4A16PerChannelFakeQuantLinear(AutoFakeQuantLinear):
    """
    W4A16 量化方式的伪量化IR。
    ...
    W4A16 量化方式可以用以下参数描述：
        ...
        weight: 权重张量，类型为torch.Tensor, dtype本质为int4, 但在ir中是torch.int8, 于保存时打包
    ...
    def __repr__(self) -> str:
        return f"W4A16PerChannelFakeQuantLinear(symmetric={self.w_sym})"

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        w_q_param = QParam(
            scheme=QScheme(scope=QScope.PER_CHANNEL, dtype=QDType.INT4, symmetric=self.w_sym), 
