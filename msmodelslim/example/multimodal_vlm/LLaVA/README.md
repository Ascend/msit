...
|------------|-------------|---------|----------------|
| LLaVA-v1.5-7B | [llava-1.5-7b-hf](https://huggingface.co/llava-hf/llava-1.5-7b-hf/tree/main) | W8A8静态量化 | MindIE当前不支持<br>vLLM Ascend当前不支持 |
...
### 使用案例
- 如果需要使用npu多卡量化，请先配置环境变量以支持多卡量化（Atlas 300I Duo 系列产品不支持）：
  ```shell
  # 根据实际情况选择多卡，以下2卡量化为例：
...
| w_bit | 权重量化bit | 8 | 可选参数;<br>当前在LLaVA量化场景下支持配置为8。|
| a_bit | 激活值量化bit | 8 | 可选参数;<br>当前在LLaVA量化场景下支持配置为8。|
