...
### 使用案例
- 如果需要使用npu多卡量化(特别是InternVL2-40B这种大模型)，请先配置多卡环境变量（Atlas 300I Duo 系列产品不支持）：
  ```shell
  # 根据实际情况选择多卡，以下8卡量化为例：
...
生成InternVL2-40B模型量化权重，异常值抑制使用m2算法（当前仅支持m2），在NPU上运行，请将{浮点权重路径}和{量化权重路径}替换为用户实际路径。{校准图片路径}在示例中默认为"./textvqa_val"，需要手动下载对应的textvqa数据集。
  ...
  ```
  ```python
  # 若使用32G显存的NPU，并且出现由于显存分布不均导致Out of memory的现象，可在模型加载时增加显存限制，示例代码如下：
...
| w_bit | 权重量化bit | 8 | 可选参数;<br>当前在InternVL 2.0量化场景下支持配置为8。|
| a_bit | 激活值量化bit | 8 | 可选参数;<br>当前在InternVL 2.0量化场景下支持配置为8。|
