...
### 使用案例
- 如果需要使用npu多卡量化(特别是Qwen2.5-VL-72B这种大模型)，请先配置多卡环境变量（Atlas 300I Duo 系列产品不支持）：
  ```shell
...
| w_bit | 权重量化bit | 8 | 可选参数;<br>当前在Qwen2.5-VL量化场景下支持配置为4或8。|
| a_bit | 激活值量化bit | 8 | 可选参数;<br>当前在Qwen2.5-VL量化场景下支持配置为8。|
...
| is_lowbit | 是否使用稀疏量化的lowbit算法 | False | 可以配置为True或者False。 <br>设置为True时使用稀疏量化的lowbit算法，反之则不使用。 <br>在`w4a8_dynamic per-group`量化场景下需要设置为True。|
