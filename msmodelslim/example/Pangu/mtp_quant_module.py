...
class SharedHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.norm = PanguUltraMoERMSNorm(
...
class MTPLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.enorm = PanguUltraMoERMSNorm(
            config.hidden_size, epsilon=config.rms_norm_eps
        )
        self.hnorm = PanguUltraMoERMSNorm(
            config.hidden_size, epsilon=config.rms_norm_eps
        )
...
class MTPModel(PreTrainedModel):
    ...
    def forward(
        ...
        hidden_states_mtp = self.mtp_layer.hnorm(hidden_states)
        ...
        hidden_states_mtp = self.mtp_layer.shared_head(layer_outputs_mtp[0])
        logits_mtp = self.lm_head(hidden_states_mtp)
        ####################### MTP LAYER ######################
...
def warp_mtp_model(config, base_model, model_path):
    ...
    mtp_safetensor = os.path.join(model_path, "model-00062-of-000062.safetensors")
