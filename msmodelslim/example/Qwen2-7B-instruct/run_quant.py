...
sys.path.insert(0, '/dl/l00845740/1220msit/msit/msmodelslim')
...
def parse_args():
    ...
    parser.add_argument("--model_path", type=str, default="/dl/model_weight/qwen2-7b-instruct",
                        help="The path to model float weights")
    parser.add_argument("--save_path", type=str, default="/dl/l00845740/qwen2-7b-lut-C8",
                        help="The path to save quant weights")
    parser.add_argument("--anti_prompt", type=str, default="/usr/local/Ascend/llm_model/examples/models/qwen/anti_prompt_qwen7b.json", help="The path to load jsonl")
    parser.add_argument("--calib_prompt", type=str, default="/usr/local/Ascend/llm_model/examples/models/qwen/anti_prompt_qwen7b.json", help="The path to load jsonl")
...
if __name__ == "__main__":
    ...
    #with open("/usr/local/Ascend/llm_model/examples/models/qwen/anti_prompt_qwen7b.json", "r") as file:
    ...
    model.anti_method = 'm6'

    disable_names = ['model.layers.1.mlp.down_proj', 'model.layers.3.mlp.down_proj',
                     'model.layers.2.mlp.down_proj', 'model.layers.4.mlp.down_proj',
                     'model.layers.27.mlp.down_proj', 'model.layers.0.mlp.down_proj',
                     'model.layers.5.mlp.down_proj', 'model.layers.26.mlp.down_proj',
                     'model.layers.1.self_attn.v_proj', 'model.layers.1.self_attn.q_proj',
                     'model.layers.1.self_attn.k_proj', 'model.layers.9.mlp.down_proj',
                     'model.layers.23.mlp.down_proj', 'model.layers.25.mlp.down_proj',
                     ...
                     'model.layers.1.mlp.up_proj', 'model.layers.1.mlp.gate_proj',
                     'model.layers.8.mlp.down_proj', 'model.layers.6.mlp.down_proj',
                     'model.layers.20.mlp.down_proj', 'model.layers.2.mlp.up_proj',
                     'model.layers.2.mlp.gate_proj', 'model.layers.19.mlp.down_proj']
    ...
    precision_test = PrecisionTest(model, tokenizer, "ceval_5_shot", 1, "npu")
    precision_test.test()
    ...
    for key, item in data.items():
        if key.split(".")[-1] in ["weight_scale"]:
            ...
            data[key] = torch.stack((item, zeros), dim=1).reshape(-1, 1)
