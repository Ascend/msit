...
class VlmSafeGenerator(SafeGenerator):
    ...
    @staticmethod
    def modify_config(model_dir, dest_dir, torch_dtype, quantize_type, args=None, quantize_config_parts=[]):
        """修改配置文件"""
        # 验证路径
        ...
        dest_quant_description_filepath = VlmSafeGenerator._get_quantization_filename(
            dest_dir, quantize_type, getattr(args, 'mindie_format', False)
        )
        ...
        # 处理mindie格式
        if args and getattr(args, 'mindie_format', False):
            data['quantize'] = quantize_type
            for config_part in quantize_config_parts:
                data[config_part]['quantize'] = quantize_type
        
        # 构建量化配置
        quantization_config = VlmSafeGenerator._build_quantization_config(args)
