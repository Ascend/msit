...
if __name__ == "__main__":
    ...
    # get anti_dataset
    with open('anti_prompt_llama8b_kmeans_c8.json', 'r') as file:
    ...
    # get calib_dataset
    with open("calib_prompt_llama8b_kmeans_c8.json", "r") as file:
    ...
    layer_selector = LayerSelector(model, dataset_calib)
    layer_selector.run_sample()
    layer_params = layer_selector.layer_names
    layer_params = (layer_param[1] for layer_param in layer_params)
    disable_names = list(layer_params)[:DISABLE_NUMS]
    ...
    for name, mod in model.named_modules():
        if isinstance(mod, torch.nn.Linear):
            for key in keys:
                if key in name:
                    disable_anti_names.append(name)
