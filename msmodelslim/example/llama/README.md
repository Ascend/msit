...
- LLaMa3.1 70B推荐使用以下配置进行PDMIX量化
  ```shell
  python3 convert_llama3.1_70b_pdmix_revert_down_only.py --model_path {浮点权重路径} --save_path {W8A8PDMIX权重路径}
  ```
- LLaMa3.1 70B 无回退权重生成，对话正常，建议仅用于基准性能测试
  ```shell
  python3 convert_llama3.1_70b_pdmix_revert_down_only.py --model_path {浮点权重路径} --save_path {W8A8PDMIX权重路径} --no_disable
  ```