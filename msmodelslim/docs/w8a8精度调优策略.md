...
# 背景
W8A8量化精度调优策略是结合modelslim量化工具和精度测试工具precision tool进行精度验证和调优开展。大模型经过W8A8量化后精度损失大，可以参考本文的精度调优策略进行调优。
...
def get_calib_dataset(tokenizer, calib_list, device=f"npu:{device_id}"):
    ...
    return calib_dataset
```
注： 需要将msmodelslim文件夹下的[precision_tool文件夹](../precision_tool/precision_tool.py)复制一份出来，和量化脚本放置于同一目录下，再将待测试数据集放入precision_tool文件夹中，具体操作见：[Precision Tool 使用方法说明及数据集下载链接](https://gitcode.com/Ascend/msit/tree/master/msmodelslim/precision_tool)  
...
[FA量化使用说明](https://gitcode.com/Ascend/msit/blob/dev/msmodelslim/docs/FA%E9%87%8F%E5%8C%96%E4%BD%BF%E7%94%A8%E8%AF%B4%E6%98%8E.md)  
