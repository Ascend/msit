...
- **MoE架构支持**：支持混合专家（MoE）架构的转换和量化。
- **逐层处理**：采用逐层量化策略，显著减少大模型量化时的显存消耗。
- **多种数据集格式**：支持图像目录和JSONL格式的校准数据集。

**适用模型类型**:

- Qwen3-VL-MoE系列：Qwen3-VL-235B-A22B等多模态MoE模型
- 其他多模态VLM模型：待后续逐步支持

**配置特点**:

- 支持`dataset`字段配置校准数据集，可以是图像目录路径或JSONL文件路径
...
| 格式类型 | 说明 | 示例 |
|---------|------|------|
| 短名称 | lab_calib目录下的数据集名称 | `"calibImages"` → `lab_calib/calibImages/` |
| 图像目录 | 包含图像文件的目录路径（支持相对路径和绝对路径） | `"/path/to/images"` 或 `"./images"` |
| JSONL文件 | JSONL格式的数据集文件（支持相对路径和绝对路径） | `"/path/to/dataset.jsonl"` |

**JSONL格式说明**:

JSONL文件每行包含一个JSON对象，格式如下：
...
字段说明：
- `image`: 图像文件路径（必需）
- `text`: 提示文本（可选，默认为"Describe this image in detail."）
...
```yaml
spec:
  dataset: "calib_data.jsonl"  # 使用JSONL格式数据集
```
