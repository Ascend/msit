...
- **模型结构**: 基于Transformer + MoE架构，视觉编码器整体加载，文本解码器逐层处理
- **MoE转换**: 自动将3D融合权重转换为标准nn.Linear层，适配量化框架
- **量化策略**: 支持W8A8混合量化（MoE专家层动态量化 + 其他层静态量化）
- **内存优化**: 通过逐层量化显著降低内存占用，支持大规模MoE模型量化
- **数据集支持**: 支持图像目录和JSONL格式的多模态校准数据集
- **配置示例**: 参考[qwen3_vl_moe_w8a8.yaml](../../../../lab_practice/qwen3_vl_moe/qwen3_vl_moe_w8a8.yaml)

**技术亮点**:
- 视觉编码器一次性加载和量化，确保图像特征提取的完整性
- 文本解码器采用逐层加载策略，按需加载每一层并立即进行量化处理
- MoE层自动识别并转换，无需手动干预
