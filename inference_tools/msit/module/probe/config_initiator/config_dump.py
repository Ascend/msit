...
class DumpConfig(BaseConfig):
    def check_config(self, dump_path: str = None):
    ...
    def _check_dump_dic(self, dump_path: str = None):
        ...
        self._update_config(
            ...
            DumpConst.LIST,
            valid_list,
            (self.task_config.get(DumpConst.LIST, []), self.config.get(CfgConst.LEVEL)),
        )
        self._update_config(
            self.task_config, DumpConst.DATA_MODE, valid_data_mode, self.task_config.get(DumpConst.DATA_MODE, ["all"])
        )
        self._update_config(
            self.task_config, DumpConst.DUMP_EXTRA, valid_dump_extra, self.task_config.get(DumpConst.DUMP_EXTRA, [])
        )
        self._update_config(
            self.task_config, DumpConst.DUMP_TIME, valid_dump_time, self.task_config.get(DumpConst.DUMP_TIME, "3")
        )
        self._update_config(self.task_config, DumpConst.OP_ID, valid_op_id, self.task_config.get(DumpConst.OP_ID, []))
        self._update_config(
            self.task_config,
        ...
        self._update_config(
            self.task_config, DumpConst.DEVICE, valid_device, self.task_config.get(DumpConst.DEVICE, "")
        )
        self._update_config(self.task_config, DumpConst.INPUT, valid_input, self.task_config.get(DumpConst.INPUT, []))
        self._update_config(
            self.task_config,
        ...
        self._update_config(
            ...
            valid_saved_model_tag,
            self.task_config.get(DumpConst.SAVED_MODEL_TAG, ["serve"]),
        )
        self._update_config(
            self.task_config,
            DumpConst.SAVED_MODEL_SIGN,
            valid_saved_model_signature,
            self.task_config.get(DumpConst.SAVED_MODEL_SIGN, "serving_default"),
        )
        self._update_config(
            self.task_config, DumpConst.WEIGHT_PATH, valid_weight_path, self.task_config.get(DumpConst.WEIGHT_PATH, "")
        )
