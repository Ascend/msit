...
class TestValidators(unittest.TestCase):
    ...
    def test_valid_data_mode_none(self):
        result = valid_data_mode([])
        self.assertEqual(result, [])

    @patch("msit.module.probe.config_initiator.validate_params.DumpConst.ALL_DATA_MODE", ["mode1"])
    def test_valid_data_mode_valid(self):
        result = valid_data_mode(["mode1"])
        self.assertEqual(result, ["mode1"])

    @patch("msit.module.probe.config_initiator.validate_params.DumpConst.ALL_DATA_MODE", ["mode1"])
    def test_valid_data_mode_invalid(self):
        with self.assertRaises(MsitException):
            valid_data_mode(["invalid"])
    ...
    @patch("msit.module.probe.config_initiator.validate_params.DumpConst.ALL_DATA_MODE", ["mode1", "mode2"])
    def test_invalid_data_mode_more_element(self):
        with self.assertRaises(MsitException):
            valid_data_mode(["mode1", "mode2"])
...
class TestValidInputAndOfflineModelInput(unittest.TestCase):
    ...
    def test_check_input_path_valid(self, mock_path):
        ...
        self.assertIsNone(OfflineModelInput([{}])._check_input_path({"path": "input.npy"}, "input1"))
    ...
    def test_draw_shape_and_path_dynamic_valid(self):
        ...
        input_data = [{"name": "x", "dym_shape": [[1], [2]]}, {"name": "y", "dym_shape": [[3], [4]]}]
    ...
    def test_draw_shape_and_path_dynamic_invalid(self):
        input_obj = OfflineModelInput([{}])
        input_obj.is_need_expand_shape = True
        input_data = [{"name": "x", "dym_shape": [[1], [2], [3]]}, {"name": "y", "dym_shape": [[4], [5]]}]
