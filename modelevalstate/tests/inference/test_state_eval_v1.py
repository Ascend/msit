...
from modelevalstate.inference.data_format_v1 import ModelOpField, ModelStruct, ModelConfig, MindieConfig, EnvField, \
    HardWare, RequestField, BatchField
...
def predict_with_model(config: PredictConfig):
    ...
    custom_encoder = CustomLabelEncoder(preset_category_data, save_dir=config.ohe_path)
    custom_encoder.fit()
    data_processor = config.dataset_type(custom_encoder)
    ...
    for ind, row in config.lines_data.iterrows():
        ...
        setattr(_cur_node, config.train_field, float(batch_data[-1]))
        ...
        if _up != -1:
            setattr(_cur_node, config.train_field, _up)
        else:
            setattr(_cur_node, config.train_field, _ud)
        predict_data.append(_cur_node)
    # 绘制结果图
    all_up, all_ud = PretrainModel.get_up_ud(tuple(predict_data), config.train_field)
    origin_up, origin_ud = PretrainModel.get_up_ud(tuple(origin_data), config.train_field)
    AnalysisState.plot_input_velocity_with_predict(origin_up, all_up, "batch_prefill",
                                                   f"origin predict Up {config.train_field} std velocity",
                                                   "batch_prefill", "velocity", save_path=config.save_path)
    AnalysisState.plot_input_velocity_with_predict(origin_ud, all_ud, "batch_decode",
                                                   f"origin predict Ud {config.train_field} std velocity",
                                                   "batch_decode", "velocity", save_path=config.save_path)
    all_up, all_ud = PretrainModel.get_decode_and_prefill_time(tuple(predict_data), config.train_field)
    origin_up, origin_ud = PretrainModel.get_decode_and_prefill_time(tuple(origin_data), config.train_field)
    AnalysisState.plot_input_velocity_with_predict(origin_up, all_up, "batch_prefill",
                                                   f"origin predict Up {config.train_field} std time", "batch_prefill",
                                                   "time", save_path=config.save_path)
    AnalysisState.plot_input_velocity_with_predict(origin_ud, all_ud, "batch_decode",
                                                   f"origin predict Ud {config.train_field} std time", "batch_decode",
                                                   "time", save_path=config.save_path)
...
def test_state_eval():
    file_paths = [Path(r"..\ModelEvalState\data\v1\llama3-8b1226-12\feature.csv")]
    base_path = Path(r"..\state_eval\tmp\pd_content\train\116")
    xgb_model_path = base_path.joinpath("../base/xgb_model.ubj")
